...
def _make_spj(spj_store, driver, truck, complete_with_evidence=True,
    ...
    if complete_with_evidence:
        spj_store.complete_stop(spj.spj_id, 0, evidence={
            "arrival": {"photo_name": "a.jpg"}, "weighing": [],
            "officer": {"photo_name": "p.jpg", "name": "X"}})
        return spj
    if close_by == "override":
        spj_store.complete_stop(spj.spj_id, 0, actor="supervisor",
                                override={"reason": "Evidence lost in the field"})
        return spj
    # Legacy record shape: completed without evidence and without an override.
    stored = spj_store.get(spj.spj_id)
    stored.stops[0].status = "completed"
    stored.stops[0].completed_at = stored.created_at
    stored.status = "selesai"
    stored.completed_at = stored.created_at
    spj_store._commit(stored)
    return spj
